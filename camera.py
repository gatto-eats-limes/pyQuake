...
class Player:
    ...
    def resolve_collision(self, min_bound, max_bound):
        ...
        # Resolve collision by moving the player out of the collision along the smallest axis
        if overlap_x < overlap_y and overlap_x < overlap_z:
            if self.position[0] < (min_bound[0] + max_bound[0]) / 2:
                self.position[0] -= overlap_x
            else:
                self.position[0] += overlap_x
            self.velocity[0] = 0  # Stop horizontal movement along the x-axis
            ...
            self.grounded = True  # Ensure player is grounded if collision is resolved vertically
        else:
            if self.position[2] < (min_bound[2] + max_bound[2]) / 2:
                self.position[2] -= overlap_z
            else:
                self.position[2] += overlap_z
            self.velocity[2] = 0  # Stop horizontal movement along the z-axis
